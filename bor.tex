...
{(d) Pitää oleman lainvastainen henkilön myödä tai toisaalta siirtää tuliaseeta tai ampumatarvikkeeta henkilölle tietäen tai järkevällä syyllä uskoa, että sellainen henkilö~---}
{(d) ---}
{(d) !`no habla español!}
...
{(4) on \textbf{tuomittu mieliviaksi} \cite{l2015}, tai on \textbf{pakottu mielisairaalaan};~---}
{(4) ---}
{(4) !`no habla español!}
...
{Ei henkilöä pidä pidettämän vastuussa kuolemalla rangaistettavasta taikka toisaalta törkeästä rikoksesta, ellei Suurelta Valamiehistöltä syytä löytämisellä eli syytteeseen pantamisella,}
...
{eikä pidä mitään henkilöä altistettaman samasta rikoksesta kahdesti panna vaaraan elämästä taikka jäsenestä;}
...
{ni imposées des amendes excessives,}
{!`no habla español!}
...
{ni infligées des punitions cruelles et anormales.}
{!`no habla español!}
...
{pidä tuleman olemaan Yhdys-Valtojen sisälle, taikka mihinkään niiden toimivallalle altistetuun paikkaan.}
...
{Kaikkia miespuolisia lapsia, Yhdys-Valloissa syntyneitä, täytyy ympärileikata esinahkan lihassa heti syntymällä.}
